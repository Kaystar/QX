...
通知模块会从脚本目录、上层目录、QL_DIR/scripts 和 /ql/scripts 查找
notify.py 或 sendNotify.py，并自动调用 send/sendNotify/send_notify。
...
===== 可选环境变量 =====
  ...
  AGENTROUTER_DEFAULT_AWARD_USD=25 无法提取奖励时显示的默认奖励美元金额
...
DEFAULT_AWARD_USD = _env_float("AGENTROUTER_DEFAULT_AWARD_USD", 25.0, 0.0, 1_000_000.0)
DEFAULT_AWARD_QUOTA = DEFAULT_AWARD_USD * QUOTA_PER_UNIT
...
def _notify_search_dirs():
    """按脚本目录、上层目录、工作目录和青龙 scripts 目录依次查找。"""
    dirs = []
    try:
    ...
    if ql_dir:
        dirs.append(Path(ql_dir) / "scripts")
    dirs.append(Path("/ql/scripts"))
...
def _sender_from_module(module, source):
    preferred = ("sendNotify", "send", "send_notify") if "sendnotify" in source.lower() else (
        "send", "sendNotify", "send_notify",
    )
    for attr in preferred:
        sender = getattr(module, attr, None)
        if callable(sender):
            return sender, f"{source}::{attr}"
    return None, None


def _load_notify_module(path):
    module_name = f"_agentrouter_notify_{abs(hash(str(path.resolve())))}"
...
def _find_notify_sender(search_dirs=None):
    """搜索 notify.py / sendNotify.py，返回 (可调用对象, 来源)。"""
    dirs = [Path(item) for item in search_dirs] if search_dirs is not None else _notify_search_dirs()
    seen_files = set()
    for directory in dirs:
        for filename in ("notify.py", "sendNotify.py"):
            ...
            seen_files.add(key)
            try:
                module = _load_notify_module(resolved)
            except Exception as exc:
                log(f"通知模块加载失败 {resolved}: {compact_text(exc)}")
                continue
            sender, source = _sender_from_module(module, str(resolved))
            if sender:
    ...
    for module_name in ("notify", "sendNotify"):
        try:
            ...
            continue
        sender, source = _sender_from_module(module, module_name)
        if sender:
            return sender, source
    return None, None


def get_notify_sender():
    global notify_sender, notify_source, notify_load_attempted
    if notify_load_attempted:
        return notify_sender, notify_source
    notify_load_attempted = True

    notify_sender, notify_source = _find_notify_sender()
    if notify_sender:
        log(f"已加载通知模块: {notify_source}")
    else:
        log("未找到 notify.py 或 sendNotify.py，通知内容将只打印到任务日志")
    return notify_sender, notify_source
...
def password_login(account):
    ...
    try:
        with requests.Session() as session:
            ...
            if checkin_award is None:
                checkin_award = DEFAULT_AWARD_QUOTA
